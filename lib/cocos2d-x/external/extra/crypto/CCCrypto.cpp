...
void CCCrypto::MD5(void* input, int inputLength, unsigned char* output)
{
    MD5_CTX ctx;
    _MD5_Init(&ctx);
    _MD5_Update(&ctx, input, inputLength);
...
LUA_STRING CCCrypto::RC4Lua(const char* input, int inputLength, const char* key, int keyLength)
{
	char* buffer = new char[inputLength];
	memcpy(buffer, input, inputLength);
	RC4encode(buffer, inputLength, key, keyLength);

	CCLuaStack* stack = CCLuaEngine::defaultEngine()->getLuaStack();
	stack->clean();

	stack->pushString((const char*)buffer, inputLength);
	delete buffer;
