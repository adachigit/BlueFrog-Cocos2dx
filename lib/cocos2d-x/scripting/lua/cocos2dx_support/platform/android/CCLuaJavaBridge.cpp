...
int CCLuaJavaBridge::callLuaFunctionById(int functionId, const char *arg)
{
    ...
    if (!lua_istable(L, -1))
    {
        LOGD("%s", "callLuaFunctionById() - LUAJ_REGISTRY_FUNCTION not exists");
        lua_pop(L, 1);
        return -1;
    }
...
int CCLuaJavaBridge::callLuaGlobalFunction(const char *functionName, const char *arg)
{
	...
	{
		LOGD("callLuaGlobalFunction() - can not find function %s or it's not a function", functionName);
	}
...
int CCLuaJavaBridge::retainLuaFunction(lua_State *L, int functionIndex, int *retainCountReturn)
{
    ...
    LOGD("retainLuaFunction() - function id %d retained", functionId);
    return functionId;
}
