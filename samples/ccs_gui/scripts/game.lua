...
function createLayer()
    ...
    local slider = ccs.slider({
        ...
        x   = s.width/2,
        y   = label:getPositionInCCPoint().y - 20,
        })
    local slider2 = ccs.slider({
        ...
        listener = {[ccs.SliderEventType.changed] = sliderCallBack},
        x = s.width/2,
        y = slider:getPositionInCCPoint().y - 30,
        })

    local loadingbar1 = ccs.loadingbar({
        image = "sliderProgress.png",
        x = s.width/2,
        y = slider2:getPositionInCCPoint().y - 30,
        })

    local loadingbar2 = ccs.loadingbar({
        image = "sliderProgress.png",
        x = s.width/2,
        y = loadingbar1:getPositionInCCPoint().y - 30,
    ...
    local textField = ccs.textField({
        ...
        x = s.width/7*6,
        y = s.height/2,
        listener = { [ccs.TextFiledEventType.attach_with_ime] = function(uiwidget) SET_POS(uiwidget,uiwidget:getPositionInCCPoint().x,uiwidget:getPositionInCCPoint().y+10) end,
                    [ccs.TextFiledEventType.detach_with_ime] = function(uiwidget) SET_POS(uiwidget,uiwidget:getPositionInCCPoint().x,uiwidget:getPositionInCCPoint().y-10) end,
...
function game.next()
    ...
    local listView = ccs.listView({
        ...
        x = panel2:getSize().width + panel2:getPositionInCCPoint().x + 5,
        y = s.height - 100,
    ...
    local listView2 = ccs.listView({
        ...
        x = listView:getSize().width + listView:getPositionInCCPoint().x + 5,
        y = listView:getPositionInCCPoint().y,
        listener = {[ccs.ListViewEventType.updateChild] = function(uiwidget)
    ...
    local pageView = ccs.pageView({
        size = CCSizeMake(110,100),
        x = 0,
        y = panel:getPositionInCCPoint().y - 100,
    ...
    local dragPanel = ccs.dragPanel({
        ...
        scale9  = true,
        image   = "green_edit.png",
        x       = listView2:getPositionInCCPoint().x,
        y       = listView2:getPositionInCCPoint().y - 100,
...
function game.testScene()
    local scene = CCScene:create()
    local node = SceneReader:sharedSceneReader():createNodeWithSceneFile("testscene/battle.json")
    scene:addChild(node)
    sharedDirector:replaceScene(scene)
end
